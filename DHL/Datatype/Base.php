    ...
    protected function validateParameterValue($key, $value)
    {
        foreach ($this->_params[$key] as $type => $typeValue)
        {
            switch ($type)
            {
                ...
                case 'length':
                    if (mb_strlen($value) != $typeValue)
                    {
                        throw new \InvalidArgumentException('Field ' . $key . ' has a size of ' . mb_strlen($value) . ' and it should be that size : ' . $typeValue);
                    }
                break;
                ...
                case 'maxLength':
                    if (mb_strlen($value) > $typeValue)
                    {
                        throw new \InvalidArgumentException('Field ' . $key . ' has a size of ' . mb_strlen($value) . ' and it cannot exceed this size : ' . $typeValue);
                    }
                break;
                ...
                case 'minLength':
                    if (mb_strlen($value) < $typeValue)
                    {
                        throw new \InvalidArgumentException('Field ' . $key . ' has a size of ' . mb_strlen($value) . ' and it cannot be less than this size : ' . $typeValue);
                    }
                break;
                ...
                case 'pattern':
                    $matches = array();
                    $typeValue = "/" . $typeValue . "/";
                    if (1 !== preg_match($typeValue, $value, $matches) || (mb_strlen($value) > 0 && !$matches[0]))
                    {
